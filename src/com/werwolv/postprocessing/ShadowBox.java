package com.werwolv.postprocessing;
...
public class ShadowBox {
     ...
     * @param player          - the in-game camera.
     */
    public ShadowBox(Matrix4f lightViewMatrix, EntityPlayer player) {
    ...
    public void update() {
        ...
        Vector3f forwardVector = Maths.toVector3f(rotation.transform(FORWARD));

        Vector3f toFar = new Vector3f(forwardVector);
        toFar.mul(SHADOW_DISTANCE);
        Vector3f toNear = new Vector3f(forwardVector);
        toNear.mul(RendererMaster.NEAR_PLANE);
        Vector3f centerNear = toNear.add(player.getPosition());
        Vector3f centerFar = toFar.add(player.getPosition());
    ...
    public Vector3f getCenter() {
        ...
        return Maths.toVector3f(invertedLight.transform(cen));
    }
    ...
    private Vector4f[] calculateFrustumVertices(Matrix4f rotation, Vector3f forwardVector,
        ...
        Vector3f upVector = Maths.toVector3f(rotation.transform(UP));
    ...
    private Matrix4f calculateCameraRotationMatrix() {
        ...
        rotation.rotate((float) Math.toRadians(-player.getYaw()), new Vector3f(0, 1, 0));
        rotation.rotate((float) Math.toRadians(-player.getPitch()), new Vector3f(1, 0, 0));
        return rotation;
    }
