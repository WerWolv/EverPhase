...
import com.werwolv.fbo.FrameBufferShadow;
import com.werwolv.model.ModelTextured;
import com.werwolv.postprocessing.ShadowBox;
...
public class RendererShadowMapMaster {
    ...
    private FrameBufferShadow shadowFbo;
     ...
     * {@link FrameBufferShadow} to which the scene is rendered. The size of the
     * shadow map is determined here.
     *
     * @param player - the camera being used in the scene.
     */
    public RendererShadowMapMaster(EntityPlayer player) {
        shader = new ShaderShadow();
        shadowBox = new ShadowBox(lightViewMatrix, player);
        shadowFbo = new FrameBufferShadow(SHADOW_MAP_SIZE, SHADOW_MAP_SIZE);
     ...
     *                 associated with the {@link ModelTextured} that all of the
     *                 entities in that list use.
     * @param sun      - the light acting as the sun in the scene.
     */
    public void render(Map<ModelTextured, List<Entity>> entities, EntityLight sun) {
        shadowBox.update();
    ...
    private void prepare(Vector3f lightDirection, ShadowBox box) {
        ...
        projectionViewMatrix = lightViewMatrix.mul(projectionMatrix);
