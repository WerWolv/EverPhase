...
public class RendererShadowMapMaster {
	 ...
	 */
	public void render(Map<ModelTextured, List<Entity>> entities, Map<ModelTextured, List<Entity>> entitiesNM, EntityLight sun) {
		shadowBox.update();
		Vector3f sunPosition = sun.getPosition();
		Vector3f lightDirection = new Vector3f(-sunPosition.x, -sunPosition.y, -sunPosition.z);
		prepare(lightDirection, shadowBox);
		entityRenderer.render(entities);
		entityRenderer.render(entitiesNM);
		finish();
	}
	...
	public Matrix4f getToShadowMapSpaceMatrix() {
		Matrix4f shadowMapSpaceMatrix = new Matrix4f();
		offset.mul(projectionViewMatrix, shadowMapSpaceMatrix);
